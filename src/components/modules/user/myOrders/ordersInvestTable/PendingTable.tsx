...
const PendingTable = ({ activeTab }: { activeTab: string }) => {
  const fields = [
  ...
  useEffect(() => {
      ...
      if (activeTab === 'Pending') {
        try {
          ...
          if (res && res?.data) {
            ...
            setPendingOrder(filteredOrders);
    ...
    fetchPendingOrder();
  }, [activeTab, fetchOrderHistory, searchQuery, page, refreshFlag, cryptoEnable]);
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `pending_orders_${Date.now()}.csv`);
  ...
  return (
    <div className="ordersinvest_table">
      <CommonFilter
        ...
        csvDisabled={!pendingOrder?.length || exportStatus === 'exporting'}
      />
      <CommonTable className="pending_table" fields={fields} lastColumnWidth="140px">
        {pendingOrder?.length > 0
          ? pendingOrder?.map((item: any, index: number) => (
              <tr key={index}>
                <td>
