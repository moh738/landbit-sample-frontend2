...
import { formatDateTime, formatCurrencyWithCrypto } from '../../../../../helpers/user/maskEmail';
...
const RejectedTable = ({ activeTab }: { activeTab: string }) => {
  const fields = [
  ...
  const [rejectedOrder, setRejectedOrder] = useState<any[]>([]);
      ...
      if (activeTab === 'Rejected') {
        try {
          ...
          if (res && res.data) {
            ...
            setRejectedOrder(filteredOrders);
  ...
  const handleDownloadCSV = async () => {
    try {
      ...
      // New flow: backend returns exportId (202), poll status then download from URL
      ...
      const rows = filteredOrders?.map((item: any) => {
        ...
        const propertyName = [item?.propertyName, item?.type].filter(Boolean).join(', ') || '-';
        const orderIdWithNft = item?.nftId ? `${item?.orderId || ''} (${prefix}- ${item.nftId})` : (item?.orderId || '-');
        const tokenQuantity = `${item?.quantity ?? 0} ${prefix} Token`;
        const amountStr = formatCurrencyWithCrypto(item?.amount, usdtPrice, cryptoEnable, item?.method);
        const feeStr = `${item?.fees ?? 0}%`;
        const dateStr = formatDateTime(item?.date) || (item?.time ? `${item?.date || ''} ${item.time}` : '-');
      ...
      link.href = url;
      link.setAttribute('download', `rejected_orders_${Date.now()}.csv`);
  ...
  return (
    <div className="ordersinvest_table">
      <CommonFilter
        ...
        csvDisabled={!rejectedOrder?.length || exportStatus === 'exporting'}
      />
      <CommonTable
        className="rejected_table"
        fields={fields}
        lastColumnWidth="140px"
      >
        {rejectedOrder?.length > 0
          ? rejectedOrder?.map((item: any, index: number) => (
              <tr key={index}>
                <td>
                ...
                <td>
                  {formatCurrencyWithCrypto(item?.amount, usdtPrice, cryptoEnable, item?.method)}
                </td>
                <td>{`${item?.fees}%`}</td>
                <td>
                  <div className="order_id">
                ...
                <td>
                  {cryptoEnable ? 'USDT' : (item?.method || 'INR')}
                </td>
                <td>
                  <span
                    className={`act-btn ${
